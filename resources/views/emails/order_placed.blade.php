    ...
    <div class="email-container gmail-specific">
        ...
        <div class="content">
            ...
            <div class="order-summary">
                ...
                <p><strong>Order Date:</strong> {{ \Carbon\Carbon::parse($order->created_at)->toFormattedDateString() }}</p>
                            ...
                            <tr>
                                <td>
                                    <div class="product-row">
                                        <img src="{{ asset('/storage/' . $item->service->thumbnail) }}" alt="{{ $item->service->name }}" class="product-image">
                                        <div class="product-info">
                                            <p class="mb-0"><strong>{{ $item->service->name }}</strong></p>
                                            <span class="text-muted">Category: {{ $item->service->category->title }}</span>
                                        </div>
                                    </div>
                                </td>
                                <td>{{ $item->quantity }}</td>
                                <td>LEI <span class="text-muted">{{ number_format($item->price, 2) }}</span></td>
                                <td>LEI {{ number_format($item->price * $item->quantity, 2) }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                        <tfoot>
            ...
            <div class="mt-4">
                <p>If you have any questions about your order, please contact our customer service team at <a href="mailto:{{ $site['email'] }}">{{ $site['email'] }}</a> or call us at {{ $site['phone'] }}.</p>
            </div>
        </div>
        ...
        <div class="footer">
            ...
            <div class="social-links">
                ...
                <a href="{{ $item->url }}" class="text-capitalize">{{ $item->name }}</a> @if($key < count($socials) - 1) | @endif
                @endforeach
            </div>
